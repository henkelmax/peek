...
public class ClientShulkerBoxTooltip implements ClientTooltipComponent {
    ...
    public int getHeight() {
        return GRID_HEIGHT * SLOT_SIZE_Y + 2 * BORDER_WIDTH + MARGIN_Y;
    }

    @Override
    public int getWidth(Font font) {
        return GRID_WIDTH * SLOT_SIZE_X + 2;
    }
    ...
    private void renderSlot(int posX, int posY, int slotId, Font font, PoseStack poseStack, ItemRenderer itemRenderer, int layer) {
        ItemStack itemStack = items.get(slotId);
    ...
    private void drawBorder(int x, int y, PoseStack poseStack, int layer) {
        ...
        blit(poseStack, x + GRID_WIDTH * SLOT_SIZE_X + BORDER_WIDTH, y, layer, Texture.BORDER_CORNER);

        for (int i = 0; i < GRID_WIDTH; i++) {
            blit(poseStack, x + BORDER_WIDTH + i * SLOT_SIZE_X, y, layer, Texture.BORDER_HORIZONTAL);
            blit(poseStack, x + BORDER_WIDTH + i * SLOT_SIZE_X, y + GRID_HEIGHT * SLOT_SIZE_Y + 1, layer, Texture.BORDER_HORIZONTAL);
        }

        for (int i = 0; i < GRID_HEIGHT; i++) {
            blit(poseStack, x, y + i * SLOT_SIZE_Y + BORDER_WIDTH, layer, Texture.BORDER_VERTICAL);
            blit(poseStack, x + GRID_WIDTH * SLOT_SIZE_X + BORDER_WIDTH, y + i * SLOT_SIZE_Y + BORDER_WIDTH, layer, Texture.BORDER_VERTICAL);
        }

        blit(poseStack, x, y + GRID_HEIGHT * SLOT_SIZE_Y + 1, layer, Texture.BORDER_CORNER);
        blit(poseStack, x + GRID_WIDTH * SLOT_SIZE_X + BORDER_WIDTH, y + GRID_HEIGHT * SLOT_SIZE_Y + 1, layer, Texture.BORDER_CORNER);
    }
